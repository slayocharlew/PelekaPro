...
{
    public function login(Request $request, ApiUserEligibility $eligibility): JsonResponse
    {
        ...
        $deviceName = trim((string) ($validated['device_name'] ?? '')) ?: 'pelekapro-client';

        $newAccessToken = DB::transaction(function () use ($user, $deviceName) {
    ...
    private function findUser(array $validated): ?User
    {
        ...
        if (isset($validated['phone'])) {
            return $query->where('phone', trim($validated['phone']))->first();
        }
